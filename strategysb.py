...
class StrategySmaDualMl(Strategy):
    ...
    def update_trading_info(self, info):
        ...
        if target is None or binance_data is None:
            return
    ...
    def get_request(self):
        ...
        try:
            ...
            if request is None:     #request가 값이 없을 때 나가
                if self.is_simulation:
                    return [
                        {
                            "id": DateConverter.timestamp_id(),
    ...
    def __create_buy(self):
        budget = self.process_unit[0]   #진입할 금액 = 예산

        if budget > self.balance:   
            budget = self.balance

        budget -= budget * self.COMMISSION_RATIO    #수수료 더하기
        price = float(self.data[-1]["closing_price"])   #진입 가격
        ...
        if (
            self.min_price > budget  #최소주문금액이 진입할 금액보다 크다면 진입 ㄴ
        ...
        #그게 아니라면
        return {
            "id": DateConverter.timestamp_id(),     #진입 id
            "type": "buy",                          #진입 buy
            "price": str(price),                    #진입가
