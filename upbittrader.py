...
class UpbitTrader(Trader):
    ...
    def __init__(
        self, budget=50000, commission_ratio=0.0005, opt_mode=True
    ):
    ...
    def get_account_info(self):
        ...
        for item in data_list:
            ...
            result["asset"][market] = {
                "average_price": int(float(item["avg_buy_price"])),
                "quantity": int(float(item["balance"]))
            }
    ...
    def _execute_order(self, task):
        ...
        #buy거나 진입가*수량이 계좌보다 크다면 ( 계좌에 돈이 없다면 )
    ...
    def _update_order_result(self, task):
        del task  # task 파라미터는 사용되지 않으므로 삭제합니다.

        # order_map에서 모든 주문의 uuid를 수집합니다.
        uuids = []
        ...
        # 주문 리스트를 조회하여 주문 상태를 업데이트합니다.
        results = self._query_order_list(uuids)
        if results is None:
            return

        # 대기 중인 요청을 저장할 딕셔너리를 초기화합니다.
        waiting_request = {}

        # 대기 중인 주문 수를 로깅합니다.
        self.logger.debug(f"waiting order count {len(self.order_map)}")

        # 각 주문에 대해 주문 결과를 업데이트합니다.
        for request_id, order in self.order_map.items():
            is_done = False
            for query_result in results:
                # 주문 결과를 조회하여 해당 주문의 uuid가 일치하는 경우
